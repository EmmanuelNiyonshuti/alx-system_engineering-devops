#!/usr/bin/python3
"""
This script fetches hot articles from a given subreddit and counts the occurrences of specific keywords in the titles.
"""
...
def print_sorted_results(keyword_count):
    """
    Prints the keyword counts sorted by count (descending) and then alphabetically.
    """
    sorted_keywords = sorted(keyword_count.items(), key=lambda item: (-item[1], item[0]))
...
def count_words(subreddit, word_list, after=None, titles=None):
    """
    Recursively Querries the Reddit API, parses titles of all hot articles,
    ...
    url = "https://www.reddit.com/r/{}/hot.json?after={}&raw_json=1".format(subreddit, after)
